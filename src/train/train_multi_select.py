...
def train_and_evaluate(
    train_data, valid_data, context_data,
    model_name_or_path, output_dir,
    learning_rate=3e-5, num_epochs=1, batch_size=2,
    max_len=512, doc_stride=128, mixed_precision="fp16"):

    # Load the datasets
    train_data = load_json(train_data)
    valid_data = load_json(valid_data)
    context_data = load_json(context_data)
